...
Vuckt is Swift library providing Obj-C-compatible integral & floating vector struct types with zero-cost SIMD vector bridging, and convenience methods to bridge to/from other vector-ish types throughout Cocoa.  Vuckt is designed to be a “plumping-level” vector library, that is, it makes as little distinction as possible about the intended purpose or usage of its types (such as a vector3 being used as a position, or as an offset, or as a normal, or as a scale, or as a euler-angles or rotor rotation, etc.) and instead focuses on just providing vector types and every vector math method could could want to operator on those vectors.  (A specialized quaternion type is present because it's not a true uniform vector4, but really a real-vector3 & imaginary1, so the math is significantly different).  Also, Vuckt is built on optimized C underpinnings, and is significantly faster than Swift's native SIMD library.
